    ...
    init: function(options) {
        ...
        pluginCenter.showApplication = function() {
                ...
                success: function(data) {
                    if (data.isSingle == 1) { //判断是否是以前购买了单个应用并且还没有应用到项目上【只有抽奖与隐藏logo有这个】
                        ...
                        if (settings.type == 'hide_logo' || settings.type == 'quota' || settings.type == 'vote_wall') {
                            if (data.buy_type == '' || data.buy_type == 'multi') {
                                pluginCenter.hideLogoRenew(data.expire_time);
                            } else {
                                pluginCenter.hidelogoBuylist(data.buy_type, data.expire_time);
                            }
            ...
            pluginCenter.sing_project_buy_exhibition = function(expire_time, is_history, is_expire) {
                ...
                if (settings.type == 'restrict_mobile_rspd') {
                    ...
                    if (is_expire) {
                        ...
                        common_text = '抽奖应用已于' + expire_time.toString() + '到期，您可以<span class="btn_again_buy" style="color:#53a4f4;cursor:pointer;" href="javascript:;">续费延期</span>再次开启抽奖应用';
                ...
                if(settings.type == 'lucky_draw'){
                    $(".common_complete").on('click', '.btn_again_buy', function(){
                        var options = {is_renew: true};
                        pluginCenter.removeChildWrap();
                        pluginCenter.appPaid(options);
                    });
            ...
            function showApplicationCreate() {
                ...
                var Immediate = "立即使用";
                if (settings.type == "lucky_draw") {
                    help_a = '<a href="http://www.wenjuan.com/helpcenter/list/53916b15f7405b30051f15b8/h3565fe439a320fc75a5479f45" target="_blank" class="cj_tip"></a>';
                } else if (settings.type == "lucky_money") {
                            ...
                            success: function(data) {
                                if (!data.info) {
                                    if (settings.project_single_id) {
                                        pluginCenter.setEnvelope(settings.project_single_id); //如果是单独项目页面上购买入口，微信红包立即使用后直接进入设置页面
                                        return false;
                                    }
        ...
        pluginCenter.appPaid = function(options) {
            var paidSettings = {
                max_month: 5,                           //最大购买月份
                buyTxt: "购买时长",                     //购买时间文案
                prevStepFuncName: "showApplication",    //点击上一步需要执行的方法名称
                nextStepFuncName: "appSubmit",          //支付的时候需要执行的方法名称
                is_renew: false,                        //是否续费
                prevText: "上一步",                     //返回按钮文案
                way: "",                                //新流程续费【暂时只有续费,也支持购买】
            };
            $.extend(paidSettings, options);
            if (paidSettings.is_renew){
                paidSettings.prevText = "返回";
            }
            var prevTextStr = '<a class="btn_prev btn_none" href="javascript:;">' + paidSettings.prevText + '</a>';
            if (paidSettings.way){
                settings.way = paidSettings.way;
                prevTextStr = '';
            }

            var currency_info = JSON.parse($.ajax({
                url: '/auth/ajax/get_currency_info/',
                "type": "POST",
                data: {
                    'currency_type': '2|3',
                    'scope': settings.type,
            ...
            var discount_str = discount_info(currency_info).discount_str,
                discount_val = 1,
                discount_arr = discount_info(currency_info).discount_arr;
            var buy_time_str = buy_time_info(paidSettings.max_month, paidSettings.buyTxt),
                buy_time_val = 12;
            var c_type = '';//优惠券类型
            var seniorUser_str = '<a class="plugin_seniorUser" href="javascript:;">升级高级版账户，即可免费使用！</a>';
            if (currency_info.super_account == 1){//已经是高级版用户将不出示广告栏
                seniorUser_str = '';
            }

            var tmp = '<div class="hide_logo_paid">' +
                '<div class="content_wrap">' +
                '<h4>请选择购买类型</h4><div class="lucky_draw_paid_box"><div class="app_youhui"></div></div>' +
                seniorUser_str +
                '<div class="buy_fitler">'+
                buy_time_str +
                discount_str +
                '</div></div>' +
                '<div class="btn_wrap">' +
                '<p class="paid_num">实付款：<span class="userMoney pink">￥<i>0.00</i></span></p>' +
                '<a class="btn_next btn_default" href="javascript:;">确认支付</a>' +
                prevTextStr +
                '</div>' +
                '</div>';
            pluginCenter.showChildWrap(settings.info.title, tmp);
            var $hideLogoPaid = $showChildWrap.find('.hide_logo_paid');
            var $discount_paid_select = $hideLogoPaid.find('.discount_paid select');    //优惠券列表
            var $buy_time_select = $hideLogoPaid.find('.buy_time_length select');       //购买时间列表
            var $discount_tip = $hideLogoPaid.find(".buy_time_length .discount_val");  //文案
            setObjContentWrap($hideLogoPaid);

            //高级版广告
            $hideLogoPaid.on('click', '.plugin_seniorUser', function() {
                parent.window.open("/senior_user/");
            });

            //购买类型
            $hideLogoPaid.on('click', '.amount_info', function() {
                if (!$(this).hasClass("disabled") && !$(this).hasClass("active")) {
                    $(this).addClass('active').siblings().removeClass('active');
                    var use_time = $(this).attr("use_time");
                    var needMoney = $hideLogoPaid.find('.amount_info.active .needMoney').text();
                    if (parseFloat(use_time) != 30) {
                        $buy_time_select.find('option:last').attr("selected", "selected");
                        $discount_tip.show();
                        buy_time_val = 12;
                    } else {
                    ...
                    init_paid_select($discount_paid_select, currency_info, needMoney, buy_time_val);
                    if (currency_info.data.length > 0) {
                        discount_val = $discount_paid_select.find('option:selected').val();
                        c_type = $discount_paid_select.find('option:selected').attr("c_type");
                    }
                    discount(needMoney, discount_val, $hideLogoPaid, buy_time_val, c_type);
                }
            });

            //购买优惠券
            $hideLogoPaid.on('change','.discount_paid select', function() {
                settings.discount_id = $discount_paid_select.find('option:selected').attr('oid');
                buy_time_val = parseInt($buy_time_select.find('option:selected').val());
                c_type = $discount_paid_select.find('option:selected').attr("c_type");
                var needMoney = $hideLogoPaid.find('.amount_info.active .needMoney').text();
                discount_val = $(this).find('option:selected').val();
                discount(needMoney, discount_val, $hideLogoPaid, buy_time_val, c_type);
            });

            //购买时间
            $hideLogoPaid.on('change', '.buy_time_length select', function() {
                buy_time_val = parseInt($(this).find('option:selected').val());
                var t_needMoney = 1;
                if (buy_time_val > paidSettings.max_month) {
                    $discount_tip.show();
                    $hideLogoPaid.find('.amount_info:last').addClass('active').siblings().removeClass('active');
                    t_needMoney = 1;
                } else {
                    $discount_tip.hide();
                    $hideLogoPaid.find('.amount_info:first').addClass('active').siblings().removeClass('active');
                    t_needMoney = buy_time_val;
                }

                var needMoney = $hideLogoPaid.find('.amount_info.active .needMoney').text();
                init_paid_select($discount_paid_select, currency_info, t_needMoney*needMoney, buy_time_val);
                if (currency_info.data.length > 0) {
                    discount_val = $discount_paid_select.find('option:selected').val();
                    c_type = $discount_paid_select.find('option:selected').attr("c_type");
                }
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    var goodsAllTmp = '';
                    $.each(goodsList, function(index) {
                        var goodsListTmp = '' +
                            '<div data-scope="' + goodsList[index].use_scope + '" id="' + goodsList[index].oid + '" use_time="' + goodsList[index].use_time + '" class="amount_info">' +
                            '<p class="num red"><!--span class="s1">￥</span--><span class="s2 needMoney">' + goodsList[index].cost + '</span><span class="s3 danwei">元/月</span></p>' +
                            '<p class="p1 del">' + goodsList[index].desc + '</p></div>';
                        goodsAllTmp += goodsListTmp;
                    });
                    $hideLogoPaid.find('.lucky_draw_paid_box').append(goodsAllTmp);
                    var paid_box = $hideLogoPaid.find('.lucky_draw_paid_box').find('.amount_info');
                    var amount_info_last = $hideLogoPaid.find('.lucky_draw_paid_box').find('.amount_info:last');
                    amount_info_last.find('.s3').html('元/年');
                    $hideLogoPaid.find('.lucky_draw_paid_box').css('width', paid_box.length * paid_box.outerWidth(true));
                    amount_info_last.trigger('click');
                }
            });
            //确认支付
            $hideLogoPaid.on('click', '.btn_next', function() {
                var settingsType = settings.type;
                if (settingsType == 'lucky_money') {
                    settingsType = 'lucky_money';
                }
                _hmt.push(['_trackEvent', 'pluginPay', 'click', settingsType +'_pluginPay']);
                var $chooseActive = $hideLogoPaid.find('.lucky_draw_paid_box .active');
                if ($chooseActive.length < 1) {
                    loadMack({
                        'str': '请选择一种类型',
                        'timer': 1500
                    });
                    ...
                    if (getAvailableRmb().status == '200') {
                        var calTypeId = $chooseActive.attr('id');
                        var scope = $chooseActive.data('scope');
                        var needMoney = parseFloat($('.paid_num .userMoney i').text());
                                    ...
                                    payment_Method.payConfirm({
                                        btnOKFn:function(){
                                            ...
                                            getOrderStatus(function(data){
                                                ...
                                                if(orderStatus == 1){
                                                    // 购买成功
                                                    pluginCenter[paidSettings.nextStepFuncName](calTypeId, paidSettings.is_renew);
            ...
            //返回上一步
            $hideLogoPaid.on('click', '.btn_prev', function() {
                if (settings.type == "restrict_mobile_rspd"){
                    //如果pid存在，那么充值页面是从控制开关页面进来的，所以上一步要回到控制开关页面
                    pluginCenter.removeChildWrap();
                    if(settings.project_single_id){
                        //限定手机广告位的简介界面
                        if(settings.string.indexOf('Advertising') != -1 && settings.isRenew != true){
                            pluginCenter.showApplication();
                        }else{
                            pluginCenter.limiteMobileComplete(settings.project_single_id, 1);
                        }
                    }else{
                        pluginCenter.showApplication();
                    }
                    ...
                    pluginCenter.removeChildWrap();
                    if(paidSettings.prevStepFuncName == "showBuyProjectList"){
                        pluginCenter.showApplication();
                        return;
                    }
                    if(paidSettings.prevStepFun){
                        paidSettings.prevStepFun();
                        return;
                    }
                    pluginCenter[paidSettings.prevStepFuncName]();
                }else if (settings.type == "custom_domain"){
                    if(paidSettings.prevText == "返回"){
                        pluginCenter.domainComplete(paidSettings.domainName, paidSettings.expireTime, paidSettings.service_status, paidSettings.audit_desc);
                    }else{
                        pluginCenter.domainStep(1);
                    }
                }else{
            ...
            $hideLogoPaid.on('click', '.btn_chongzhi', function() {
        ...
        pluginCenter.transferComplete = function(options){
            ...
            if (options.is_renew){
                sub_settings.h4Text = (settings.info.title).replace(" ", "") + "续费成功";
            }

            if (settings.type == "hide_logo"){
                if (settings.project_single_id){
                    $(".hide_logo_buy", parent.document).remove();//如果是在预览页购买，则购买完成后关闭购买窗口
                    sub_settings.transferCompleteName = "查看效果";
                }else{
                    sub_settings.transferCompleteName = "我知道了";
                }
            }else if (settings.type == "lucky_money"){
                ...
                sub_settings.transferCompleteName = "查看设置";
            }
        ...
        pluginCenter.transferCompleteFn = function(options){
            if (settings.type == "hide_logo"){
                if (settings.project_single_id){
                    window.open('/s/' + settings.project.short_id + '?test=1');
                }else{
                    // window.open('/list');
                    parent[settings.string].remove();
                }
                ...
                if (settings.project_single_id){
                    ...
                    pluginCenter.showBuyProjectList(options.expire_time);
                }
            }else if (settings.type == "wx_signin"){
                if (sessionStorage.isRenew) {
                    var sessionStorage_data = JSON.parse(sessionStorage.isRenew);
                    if (sessionStorage_data["pid"]) { //从列表进入续费
                        sessionStorage.isRenew = false;
                        pluginCenter.wxSigninComplete(sessionStorage_data);
                        return false;
                    }
                }
                pluginCenter.showBuyProjectList(options.expire_time);
                ...
                pluginCenter.domainComplete(options.domain_name, options.expire_time, options.service_status, options.audit_desc);
            }else if (settings.type == "lucky_money"){
                ...
                parent.window.location.href = '/plugin/use_list/';
            }
        };
        ...
        pluginCenter.wxSigninComplete = function(data) {
            ...
            var headStr = '';
            if (!is_expire) {
                headStr = '<h2>微信签到应用已启用</h2><div class="tip">用户必须通过微信填写表单，使用微信扫一扫 [ 签到二维码 ] 完成签到<br/>系统已默认开启“用户只能通过微信填写”，可在“签到设置”中修改</div>';
            } else {
                headStr = '<h2>微信签到应用<span class="red">已到期</span></h2><div class="tip">用户无法使用微信扫码签到，启用签到功能请<a class="blue btn_renew">立即续费</a></div>';
            }
        ...
        pluginCenter.domainAudit = function(mobileNumber, hasPrev) {
            ...
            var auditStr = '<div class="domain_audit">' +
                ...
                '<p>您的域名<span class="blue">' + domainName + '</span>已提交审核</p>' +
                '<p>我们会在24小时内进行审核，审核通过后您可以进行付款操作</p>' +
                '</div>' +
        ...
        pluginCenter.showBuyProjectList = function(expire_time, is_expire) {
            ...
            buyAgain = '应用已在下列项目使用，请查看';
            var appTxtDict = {'lucky_draw':'抽奖','restrict_mobile_rspd':'限定手机号答题'}
            if (settings.type == 'lucky_draw' || settings.type == 'restrict_mobile_rspd') {
                buyAgain = '应用已在下列项目使用，请查看或 <a class="btn_again_buy" href="javascript:;">再次购买</a>';
                if (expire_time && typeof(expire_time) != "undefined") {
                    buyAgain = '应用截止日期：' + expire_time.toString() + ' <a class="btn_again_buy" href="javascript:;">[续费延期]</a>';
                    if (is_expire) {
                        buyAgain = '应用已于' + expire_time.toString() + '到期，<a class="btn_again_buy" href="javascript:;">[立即续费]</a>重新开启'+ appTxtDict[settings.type] +'应用';
                ...
                buyAgain = '<span class="wxsign_tip">提示：微信签到仅支持表单，问卷将在后续版本中支持</span><span class="wx_signin_tab"><a class="btn_tab_default active" isEnabled="true">已启用</a><a class="btn_tab_default" isEnabled="false">未启用</a></span><span class="wx_signin_renew_tip">应用截止日期：' + expire_time.toString() + '<a class="btn_renew">[续费]</a></span>';
            }
            ...
            if(settings.type == 'lucky_draw' || settings.type == 'restrict_mobile_rspd'){
                $showBuyProjectList.on('click', '.title .btn_again_buy', function(){
                    var options = {is_renew: true};
                    pluginCenter.removeChildWrap();
                    pluginCenter.appPaid(options);
                });
            ...
            if (settings.type == "wx_signin") {
                ...
                $showBuyProjectList.on("click", ".btn_renew", function() {
                    var options = {
                        is_renew: true,
                        prevStepFuncName: "showBuyProjectList"
                    };
                    sessionStorage.isRenew = false;
                    pluginCenter.appPaid(options);
                });
                        ...
                        if (settings.type == 'wx_signin') {
                            ...
                            if (data.proj_list.length > 0) {
                                ...
                                if (sessionStorage.firstLoadWxsigninProjList == "true" && ajaxData.isEnabled == "true") {
                                    ...
                                    sessionStorage.firstLoadWxsigninProjList = false;
                                    var enabledStr = ajaxData.isEnabled == "true" ? '已启用' : '未启用';
                                    projListStr += '<li class="project_null_result">暂无' + enabledStr + '项目</li>';
                                }
                            }
                        } else {
                            $.each(data.proj_list, function(i) {
        ...
        pluginCenter.hidelogoBuylist = function(buy_type, expire_time) {
            var projectStr, curPage = 1,
                totalPage = 1,
                titleStr = '';
            if (buy_type == 'single') {
                titleStr = '<h6>下列项目已隐藏问卷网标志，可无限期使用</h6><p class="title">如需隐藏其他项目的标志，请 <a href="javascript:;" class="btn_again_buy">再次购买</a></p>';
            } else if (buy_type == 'all') {
                titleStr = '<h6>你账号下所有项目均已隐藏问卷网标志，有效期至 <span>' + expire_time + '</span></h6><p class="title">如需延长使用期，请 <a href="javascript:;" class="btn_again_buy">再次购买</a></p><p class="tip">下列项目已购买无限期使用，到期后不会受到影响</p>';
            }
            projectStr = '<div class="show_buy_project_list hidelogo_buylist">' +
                '<div class="content_wrap">' + titleStr +
                '<div class="project_wrap"><ul></ul></div>' +
                '<div class="page_wrap"><div class="c_paginationa project_list clearfix">' +
                '<div class="c_pagenum"></div>' +
                '</div></div>' +
                ...
                '</div>';
            pluginCenter.showChildWrap(settings.info.title, projectStr);
            var $hidelogoBuylist = $dialogApplication.find('.hidelogo_buylist');
            setObjContentWrap($hidelogoBuylist);

            getProList(function() {
                if (totalPage > 1) {
                    $hidelogoBuylist.find('.project_list .c_pagenum').createPage({
                        pageCount: totalPage,
                        current: curPage,
                        backFn: function(cur) {
                            curPage = cur;
            ...
            $hidelogoBuylist.on('click', '.title .btn_again_buy', function(){
                var options = {is_renew: true};
                pluginCenter.removeChildWrap();
                pluginCenter.appPaid(options);
            });
            ...
            function getProList(fn) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        totalPage = data.total_page;
                        $.each(data.proj_list, function(i) {
                            projListStr += '<li class="' + data.proj_list[i].p_type + '" short_id="' + data.proj_list[i].short_id + '"><i></i><p>' + data.proj_list[i].title + '</p><span oid="' + data.proj_list[i].oid + '" class="btn_look_record">立即查看</span></li>';
                        });
                        $hidelogoBuylist.find('.project_wrap ul').html(projListStr);
                        if (fn) fn();
                    }
                });
            };

            $hidelogoBuylist.on('click', '.btn_look_record', function() {
                var pid = $(this).attr('oid');
                window.open('/s/' + $(this).parent().attr('short_id'));
            });
        }
        ...
        pluginCenter.hideLogoRenew = function(expire_time) {
            ...
            if (nowdate > enddate) {
                overdueStr = '<h6 class="f16">已于 ' + expire_time + ' 到期，如需使用请 <a href="javascript:;" class="renew_btn_v2">再次购买</a></h6><p>' + txts.txt1 + '</p>';
            } else {
                overdueStr = '<h6>该功能有效期至<span>' + expire_time + '</span></h6><p>' + txts.txt2 + '</p><a href="javascript:;" class="renew_btn">续费延期</a>';
            }
            ...
            $hideLogoRenew.on('click', '.renew_btn_v2,.renew_btn', function(){
                var options = {is_renew: true};
                pluginCenter.removeChildWrap();
                pluginCenter.appPaid(options);
            });
        ...
        pluginCenter.luckyDrawRule = function(drawSettings) {
            ...
            $drawRule.on('click', '.btn_next', function() {
                if ($('.agree_rule').hasClass('active')) {
                    ...
                    var distributeDesc = $("#distributeDesc").val();
